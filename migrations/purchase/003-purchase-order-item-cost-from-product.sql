-- Migration: 003-purchase-order-item-cost-from-product
-- What: create_purchase_order() no longer trusts the client-supplied
--       unit_price per item. It now resolves the cost server-side from
--       general_schema.product_variant.cost_price (USD) for the given
--       tenant/product_variant, and raises if the product doesn't exist for
--       that tenant. purchase_order_item.unit_price/supplier_invoice_item
--       still store the resolved value as a snapshot (kept for history,
--       three_way_matching and general_schema.product_cost_history).
-- Why:  MBP_Cambios_CR_a_Venezuela.md seccion 3: "Costo unitario por item de
--       orden: Se elimina el campo de costo unitario a nivel de item; el
--       costo se toma directamente de la configuracion del producto en el
--       modulo General."
-- Context: Fase 3 del plan de migracion del modulo de Compras. Ultima
--       modificacion a create_purchase_order() en este plan (001, 002, 003
--       tocan la misma funcion en migraciones append-only separadas).
...
COMMENT ON COLUMN purchase_schema.purchase_order_item.unit_price IS
    'Snapshot server-side del costo del producto (general_schema.product_variant.cost_price, USD) al momento de crear la orden. No editable por request del cliente (create_purchase_order() ignora cualquier unit_price recibido en p_items).';
...
-- ─────────────────────────────────────────────────────────────────────────────
-- (restaurar create_purchase_order() a la version de 002-add-payment-due-date.sql,
--  que vuelve a tomar unit_price del payload del cliente)
